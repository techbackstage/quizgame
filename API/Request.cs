...
namespace QuizGame.API
{
    public class Request
    {
        ...
        {
            Console.WriteLine($"Starting API call for message: {message}");
            string apiToken = Request.Bareer;
            Console.WriteLine($"API Token loaded: {(string.IsNullOrEmpty(apiToken) ? "EMPTY" : "PRESENT")}");
        ...
        {{
            ...
            Console.WriteLine($"JSON Request Body: {jsonBody}");
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                return new List<Question>(); // Return empty list on error
            }
